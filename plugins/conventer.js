        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                ...
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/WhatsAsenaDuplicated/stweb.mp4'), MessageType.video, {quoted: message.data, mimetype: Mimetype.mp4, caption: 'Made by FASWEEH', quoted: message.data })
   ...
   MyPnky.addCommand({pattern: 'mp4$', desc: Lang.ANİM_STİCK, fromMe: false}, (async (message, match) => {
    ...
    var usge = ''
     if (Config.LANG == 'EN') {
        ...
        usge = '.doc inrl *replace inrl with desired name'
    }
    if (Config.LANG == 'ML') {
